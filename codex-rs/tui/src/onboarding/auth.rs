...
pub(crate) enum SignInState {
    ...
    ApiKeyConfigured,
    DeepSeekEntry(DeepSeekInputState),
    DeepSeekModelSelect(DeepSeekModelState),
    DeepSeekConfigured,
}
...
/// Read the DEEPSEEK_API_KEY environment variable.
fn read_deepseek_api_key_from_env() -> Option<String> {
...
impl DeepSeekModel {
    pub(crate) fn label(&self) -> &'static str {
        match self {
            DeepSeekModel::Chat => "deepseek-v4-flash (Flash, fast)",
            DeepSeekModel::Reasoner => "deepseek-v4-pro (Pro, deep reasoning)",
            DeepSeekModel::Both => "Use both (auto-select)",
        }
    }

    pub(crate) fn description(&self) -> &'static str {
        match self {
            DeepSeekModel::Chat => "Fast and affordable, best for everyday coding tasks",
            DeepSeekModel::Reasoner => "Advanced reasoning, better for complex problem-solving",
            DeepSeekModel::Both => "Automatically select the best model for each task",
        }
    }

    pub(crate) fn model_name(&self) -> &'static str {
        match self {
            DeepSeekModel::Chat => "deepseek-v4-flash",
            DeepSeekModel::Reasoner => "deepseek-v4-pro",
            DeepSeekModel::Both => "deepseek-v4-flash",
        }
    }
}
...
pub(crate) struct DeepSeekModelState {
    ...
    pub(crate) highlighted: DeepSeekModel,
}

/// Write the DeepSeek provider config to ~/.codex/config.toml.
///
/// Creates the file if it does not exist, or appends the DeepSeek section to an
/// existing config.
...
impl KeyboardHandler for AuthModeWidget {
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        if keys::MOVE_UP.is_pressed(key_event) {
            let is_model_select = self
                .sign_in_state
                .read()
                .is_ok_and(|guard| matches!(&*guard, SignInState::DeepSeekModelSelect(_)));
            if is_model_select {
                if let SignInState::DeepSeekModelSelect(state) =
                    &mut *self.sign_in_state.write().unwrap()
                {
                    state.highlighted = match state.highlighted {
                        DeepSeekModel::Chat => DeepSeekModel::Both,
                        DeepSeekModel::Reasoner => DeepSeekModel::Chat,
                        DeepSeekModel::Both => DeepSeekModel::Reasoner,
                    };
        ...
        if keys::MOVE_DOWN.is_pressed(key_event) {
            ...
            if is_model_select {
                ...
                {
                    state.highlighted = match state.highlighted {
                        DeepSeekModel::Chat => DeepSeekModel::Reasoner,
                        DeepSeekModel::Reasoner => DeepSeekModel::Both,
                        DeepSeekModel::Both => DeepSeekModel::Chat,
                    };
                    self.request_frame.schedule_frame();
                }
            } else {
        ...
        if keys::CONFIRM.is_pressed(key_event) {
            ...
            match sign_in_state {
                ...
                SignInState::DeepSeekEntry(state) => {
                    let trimmed = state.value.trim().to_string();
                    if trimmed.is_empty() {
                        self.set_error(Some("API key cannot be empty".to_string()));
                    } else {
                        self.start_deepseek_model_select(trimmed);
                    }
                }
                SignInState::DeepSeekModelSelect(state) => {
                    let api_key = state.api_key.clone();
                    let model = state.highlighted;
                    self.save_deepseek_config(api_key, model);
                }
                _ => {}
            }
            return;
...
impl AuthModeWidget {
    ...
    pub(crate) fn cancel_active_attempt(&self) {
        ...
        match &*sign_in_state {
            ...
            SignInState::DeepSeekEntry(_)
            | SignInState::DeepSeekModelSelect(_)
            | SignInState::DeepSeekConfigured => {}
            _ => return,
        }
    ...
    pub(crate) fn is_api_key_entry_active(&self) -> bool {
        self.sign_in_state.read().is_ok_and(|guard| {
            matches!(
                &*guard,
                SignInState::ApiKeyEntry(_) | SignInState::DeepSeekEntry(_)
            )
        })
    }
    ...
    pub(crate) fn api_key_entry_has_text(&self) -> bool {
        self.sign_in_state.read().is_ok_and(|guard| {
            matches!(&*guard, SignInState::ApiKeyEntry(state) if !state.value.is_empty())
                || matches!(&*guard, SignInState::DeepSeekEntry(state) if !state.value.is_empty())
        })
    }
    ...
    fn displayed_sign_in_options(&self) -> Vec<SignInOption> {
        ...
        options.push(SignInOption::DeepSeek);
        options
    }
    ...
    fn handle_sign_in_option(&mut self, option: SignInOption) {
        match option {
            ...
            SignInOption::DeepSeek => {
                self.start_deepseek_entry();
            }
        }
    }
            ...
            match option {
                ...
                SignInOption::DeepSeek => {
                    lines.extend(create_mode_item(
                        ...
                        option,
                        "Use DeepSeek",
                        "Fast and affordable AI-powered coding",
                    ));
                }
            }
            lines.push("".into());
    ...
    fn render_deepseek_entry(&self, area: Rect, buf: &mut Buffer, state: &DeepSeekInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec!["> ".into(), "Configure DeepSeek".bold()]),
            "".into(),
            "  Enter your DeepSeek API key below for usage-based billing.".into(),
            "".into(),
    ...
    fn render_deepseek_model_select(
        &self,
        area: Rect,
        buf: &mut Buffer,
        state: &DeepSeekModelState,
    ) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec!["> ".into(), "Select default model".bold()]),
            "".into(),
        ];
        ...
        for model in models {
            let is_selected = state.highlighted == model;
            let caret = if is_selected { ">" } else { " " };

            let line1 = if is_selected {
                Line::from(vec![format!("{caret} ").cyan(), model.label().cyan()])
            } else {
                format!("  {}", model.label()).into()
            };
            let line2 = if is_selected {
                Line::from(format!("     {}", model.description()))
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::DIM)
            } else {
                Line::from(format!("     {}", model.description()))
    ...
    fn render_deepseek_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  • API key saved".into(),
            "  • Config written to ~/.codex/config.toml".into(),
            Line::from(vec![
                "  • Default model: ".into(),
                DeepSeekModel::Chat.label().into(),
            ]),
            "".into(),
            "  Codex will use DeepSeek for usage-based billing.".into(),
        ];
    ...
    fn handle_api_key_entry_key_event(&mut self, key_event: &KeyEvent) -> bool {
...

        {
            ...
            } else if let SignInState::DeepSeekEntry(state) = &mut *guard {
                if keys::CANCEL.is_pressed(*key_event) {
                    *guard = SignInState::PickMode;
                    self.set_error(/*message*/ None);
                    should_request_frame = true;
                    ...
                    if trimmed.is_empty() {
                        ...
                        // CONFIRM handled by the main handler (transitions to model select)
                        return false;
                    }
                } else {
                    match key_event.code {
    ...
    fn start_deepseek_entry(&mut self) {
        self.set_error(/*message*/ None);
        ...
        match &mut *guard {
            ...
            _ => {
                *guard = SignInState::DeepSeekEntry(DeepSeekInputState {
                    value: prefill_from_env.clone().unwrap_or_default(),
                    prepopulated_from_env: prefill_from_env.is_some(),
                });
            }
    ...
    fn start_deepseek_model_select(&mut self, api_key: String) {
        self.set_error(/*message*/ None);
        *self.sign_in_state.write().unwrap() =
            SignInState::DeepSeekModelSelect(DeepSeekModelState {
                api_key,
                highlighted: DeepSeekModel::Chat,
            });
        self.request_frame.schedule_frame();
    }

    fn save_deepseek_config(&mut self, api_key: String, model: DeepSeekModel) {
        self.set_error(/*message*/ None);
        let sign_in_state = self.sign_in_state.clone();
        let error = self.error.clone();
        let request_frame = self.request_frame.clone();
        // Get codex_home from config path (use standard location).
        let codex_home = dirs::home_dir()
            .map(|p| p.join(".codex"))
            .unwrap_or_else(|| PathBuf::from(".codex"));
        tokio::spawn(async move {
            match write_deepseek_config(&codex_home, &api_key, model).await {
                Ok(()) => {
                    *error.write().unwrap() = None;
                    *sign_in_state.write().unwrap() = SignInState::DeepSeekConfigured;
                }
                Err(err) => {
                    *error.write().unwrap() = Some(format!("Failed to save config: {err}"));
                    *sign_in_state.write().unwrap() =
                        SignInState::DeepSeekEntry(DeepSeekInputState {
                            value: api_key,
                            prepopulated_from_env: false,
                        });
                }
...
impl StepStateProvider for AuthModeWidget {
    fn get_step_state(&self) -> StepState {
        ...
        match &*sign_in_state {
            ...
            | SignInState::ChatGptSuccessMessage
            | SignInState::DeepSeekEntry(_)
            | SignInState::DeepSeekModelSelect(_) => StepState::InProgress,
            SignInState::ChatGptSuccess
            | SignInState::ApiKeyConfigured
            | SignInState::DeepSeekConfigured => StepState::Complete,
        }
    }
}
...
impl WidgetRef for AuthModeWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        match &*sign_in_state {
            ...
            SignInState::DeepSeekEntry(state) => {
                self.render_deepseek_entry(area, buf, state);
            }
            SignInState::DeepSeekModelSelect(state) => {
                self.render_deepseek_model_select(area, buf, state);
            }
            SignInState::DeepSeekConfigured => {
                self.render_deepseek_configured(area, buf);
            }
        }
    }
}
